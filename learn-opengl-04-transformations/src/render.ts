...
const __triangleProgramUniformNames = ["uModelMatrix", "uTexture"] as const;
...
export class Renderer {
  ...
  constructor(canvas: HTMLCanvasElement, textureImage: HTMLImageElement) {
    ...
    this.#triangleModel = this.#prepareTriangle(
      ...
      // biome-ignore format:
      new Float32Array([
        1.0, 0.5, 0, // top right position
        1.0, 1.0, // top right uv coordinate

        1.0, -0.5, 0, // bottom right
  ...
  #update(dt: number): void {
    // rotate at 30deg/s
    this.#angle += ((dt / 1000) * 30 * Math.PI) / 180;

    mat4.identity(this.#modelMatrix);
    mat4.translate(this.#modelMatrix, this.#modelMatrix, new Float32Array([0.2, 0.2, 0])); // TODO: this should not be allocated over and over
    mat4.rotateZ(this.#modelMatrix, this.#modelMatrix, this.#angle);
    mat4.scale(this.#modelMatrix, this.#modelMatrix, new Float32Array([0.3, 0.3, 0.3]));
  }
  ...
  #draw(): void {
    ...
    this.#gl.uniformMatrix4fv(this.#triangleProgram.uniforms.uModelMatrix, false, this.#modelMatrix);
